...
func TestTaxComboRule01_VATEXRequired(t *testing.T) {
	...
	t.Run("exempt without any VATEX fails", func(t *testing.T) {
		...
		assert.ErrorContains(t, err, "cef-vatex extension code must be present and valid for Z/E/O categories, and must not be set for Standard")
	})
...
func TestTaxComboRule03_ValidCategoryCodes(t *testing.T) {
	...
	t.Run("category code outside the SA-allowed subset fails", func(t *testing.T) {
		...
		err := rules.Validate(inv)
		assert.ErrorContains(t, err, "untdid-tax-category extension code must contain one of the values (S, Z, E, O)")
	})
}
...
func TestTaxComboRule01_UnrecognizedCategoryPassesVATEX(t *testing.T) {
	t.Run("unrecognized tax category with VATEX still passes VATEX rule", func(t *testing.T) {
		...
		assert.NotContains(t, errString(err), "cef-vatex extension code must be present and valid")
	})
