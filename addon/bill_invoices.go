	...
	return rules.For(new(bill.Invoice),
		...
		rules.When(
			...
			rules.Field("customer",
				rules.Field("addresses",
					rules.Each(
						rules.Field("street",
							rules.Assert("10", "customer address must have a street name for standard tax invoices (BR-KSA-10)", is.Present),
						),
						rules.Field("locality",
							rules.Assert("11", "customer address must have a city name for standard tax invoices (BR-KSA-10)", is.Present),
						),
						rules.Field("country",
							rules.Assert("12", "customer address must have a country code for standard tax invoices (BR-KSA-10)", is.Present),
						),
					),
				),
			...
			rules.Field("delivery",
				rules.Assert("15", "delivery must be present for standard tax invoices", is.Present),
				rules.Field("date",
					rules.Assert("16", "delivery must have a supply date for standard tax invoices (BR-KSA-15)", is.Present),
				),
			),
		),
		...
		rules.When(
			...
			rules.Field("delivery",
				...
				rules.Field("period",
					rules.Assert("19", "supply must have a delivery period for simplified and summary invoices", is.Present),
					rules.Field("start",
						rules.Assert("20", "delivery start date must be present for simplified and summary invoices (BR-KSA-72)", is.Present),
					),
					rules.Field("end",
						rules.Assert("21", "delivery end date must be present for simplified and summary invoices (BR-KSA-72)", is.Present),
					),
				),
			),
		),
		...
		rules.When(
			...
			rules.Field("customer",
				rules.Assert("23",
					"customer must be present for standard tax invoices, simplified summary invoices, and simplified invoices with EDU or HEA exemptions",
					is.Present),
				rules.Field("name",
					rules.Assert("24",
		...
		rules.Field("supplier",
			...
			rules.Field("identities",
				rules.Assert("27", "supplier must have a valid identity (CRN/MOM/MLS/700/SAG/OTH) (BR-KSA-08)",
		...
		rules.When(
			...
			rules.Field("customer",
				...
				rules.Field("identities",
					rules.Assert("30", "customer must have a valid identity for self-billed invoices (CRN/MOM/MLS/700/SAG/OTH) (BR-KSA-08)",
						is.Func("identity must be one of: CRN/MOM/MLS/700/SAG/OTH", hasOneSupplierIdentity),
					),
				),
			),
